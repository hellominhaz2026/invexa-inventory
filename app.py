...
#    Token into TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN below (or, better,
#    set them as environment variables so they're never hard-coded).
# 3. For WhatsApp: Console → Messaging → Try it out → Send a WhatsApp
#    message. While testing, Twilio gives you a sandbox number — use it
#    as TWILIO_WHATSAPP_FROM in the form "whatsapp:+14155238886", and
#    have every recipient (including yourself) send the shown join code
#    to that number once from their own WhatsApp. For production you'd
#    apply for a real WhatsApp Business sender instead of the sandbox.
# 4. For SMS: buy/use a Twilio phone number and put it in TWILIO_SMS_FROM
#    as "+1XXXXXXXXXX" (Twilio SMS numbers can't send to Bangladesh from
#    a trial account in every case — check Twilio's country coverage).
# 5. ADMIN_PHONE is where low-stock alerts are sent, e.g. "+8801XXXXXXXXX".
#
# Nothing breaks if these are left blank — sending is simply skipped and
# a clear message is flashed instead.
TWILIO_ACCOUNT_SID = os.environ.get("TWILIO_ACCOUNT_SID", "")
TWILIO_AUTH_TOKEN = os.environ.get("TWILIO_AUTH_TOKEN", "")
TWILIO_WHATSAPP_FROM = os.environ.get("TWILIO_WHATSAPP_FROM", "")  # e.g. "whatsapp:+14155238886"
TWILIO_SMS_FROM = os.environ.get("TWILIO_SMS_FROM", "")            # e.g. "+1XXXXXXXXXX"
ADMIN_PHONE = os.environ.get("ADMIN_PHONE", "")                    # e.g. "+8801XXXXXXXXX"
...
def send_whatsapp(to_number, body):
    """Send a WhatsApp message via Twilio. Returns (ok: bool, message: str)."""
    to_number = _clean_phone(to_number)
    if not to_number:
        return False, "No phone number on file for this contact."
    if not to_number.startswith("+"):
        return False, "Phone number must be in international format, e.g. +8801XXXXXXXXX."
    client = _twilio_client()
    if client is None or not TWILIO_WHATSAPP_FROM:
        return False, "WhatsApp isn't configured yet — add your Twilio credentials in app.py."
    try:
...
def send_sms(to_number, body):
    """Send a plain SMS via Twilio. Returns (ok: bool, message: str)."""
    to_number = _clean_phone(to_number)
    if not to_number:
        return False, "No phone number on file for this contact."
    if not to_number.startswith("+"):
    ...
    if client is None or not TWILIO_SMS_FROM:
        return False, "SMS isn't configured yet — add your Twilio credentials in app.py."
    try:
        client.messages.create(from_=TWILIO_SMS_FROM, to=to_number, body=body)
        return True, "SMS sent."
    except Exception as e:
        return False, f"SMS send failed: {e}"


def notify_low_stock(name, quantity, minimum_stock):
    """Fire a WhatsApp alert (falling back to SMS) to ADMIN_PHONE the moment
    a product's stock drops to or below its minimum level. Silently does
    nothing if ADMIN_PHONE / Twilio aren't configured, or if stock is fine."""
    if not ADMIN_PHONE or quantity > minimum_stock:
        return
    status = "OUT OF STOCK" if quantity <= 0 else "LOW STOCK"
    body = (
        f"\u26a0\ufe0f {APP_NAME} {status} ALERT\n"
        f"{name}\n"
        f"On hand: {quantity} (minimum: {minimum_stock})\n"
        f"Please restock soon."
    )
    ok, _ = send_whatsapp(ADMIN_PHONE, body)
    if not ok:
...
def verify_password(plain_password, stored_password):
    ...
    Supports properly hashed passwords (the normal case) and, for backwards
    compatibility with an old inventory.db that still has plain-text
    passwords, a direct comparison — which then gets upgraded to a hash.
    """
...
def login():
    if request.method == "POST":
        ...
        if user and verify_password(password, user["password"]):
            # Transparently upgrade legacy plain-text passwords (e.g. from an
            # older inventory.db) to a proper hash the first time they're used.
...
def settings():
    ...
    if request.method == "POST":
        ...
        # Username uniqueness check (only matters if it's actually changing)
...
def create_sale():
    ...
    if request.method == "POST":
        ...
        try:
            notify_low_stock(product["name"], new_stock, product["minimum_stock"])
        except Exception:
            pass
...
@app.route("/sales/invoice/<int:sale_id>/notify/<channel>", methods=["POST"])
@login_required
def sales_invoice_notify(sale_id, channel):
    db = get_db()
    sale = db.execute("""
        SELECT sales.*, customers.name AS customer_name, customers.phone AS customer_phone
        FROM sales
    ...
    lines = [
        f"\U0001f9fe {APP_NAME} Invoice #{'%05d' % sale['id']}",
        f"Customer: {sale['customer_name'] or 'Walk-in customer'}",
        "",
    ]
    for item in items:
    ...
    if sale["discount"]:
        lines.append(f"Discount: -\u09f3{sale['discount']:,.2f}")
    lines.append(f"Total: \u09f3{sale['total_amount']:,.2f}")
    lines.append("")
    lines.append("Thank you for your business!")
    body = "\n".join(lines)

    if channel == "whatsapp":
        ok, msg = send_whatsapp(sale["customer_phone"], body)
    elif channel == "sms":
        ok, msg = send_sms(sale["customer_phone"], body)
    else:
        ok, msg = False, "Unknown notification channel."
